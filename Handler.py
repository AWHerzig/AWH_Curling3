    ...
    def playNextSlate(self, handle):
        ...
        for i in range(diffTarget//2):
            if self.blist[i] is not None:
                tarind = diffTarget - i - 2
                if len(self.blist) > tarind and self.blist[tarind] is not None:
                    result = game_LoRes(self.blist[i], self.blist[tarind])
    ...
    def playNextSlate(self, handle):
        ...
        for tier in range(3):
            ...
            for i in range(0, len(curtiers[tier]), 2):
                result = game_LoRes(curtiers[tier][i], curtiers[tier][i+1])
...
class DE4: # Basically a mini swiss lol
    def __init__(self, name, teams):
        self.name = name
        self.teams = teams
        self.tiers = [teams, [], []]
...
class DubElim32:
    ...
    def playNextSlate(self, handle):
        ...
        if self.stage >= 11:
            ...
            result = game_LoRes(self.upper[0], self.lower[0])
            ...
            if self.upper[0] == winner:
                ...
                result = game_LoRes(self.lower[0], self.upper[0])
            ...
            for i in range(0, len(uppers), 2):
                result = game_LoRes(uppers[i], uppers[i+1])
        ...
        if 1 < self.stage < 10:
            ...
            for i in range(0, len(lowers), 2):
                result = game_LoRes(lowers[i], lowers[i+1])
